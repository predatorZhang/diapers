...
/*
     UUID servUuid = Sensor.BAROMETER.getService();UUID_IRT_SERV service UUID_IRT_CONF��Ӧ����charateristics��dataҲ��һ��������charateristic
	 UUID configUuid = Sensor.BAROMETER.getConfig();
 */
public enum Sensor {
  IR_TEMPERATURE(SensorTag.UUID_IRT_SERV, SensorTag.UUID_IRT_DATA, SensorTag.UUID_IRT_CONF) {
    @Override
  ...
  HUMIDITY(SensorTag.UUID_HUM_SERV, SensorTag.UUID_HUM_DATA, SensorTag.UUID_HUM_CONF) {
    @Override
    public Point3D convert(final byte[] value) {
  ...
  SIMPLE_KEYS(SensorTag.UUID_KEY_SERV, SensorTag.UUID_KEY_DATA, null) {
    @Override
